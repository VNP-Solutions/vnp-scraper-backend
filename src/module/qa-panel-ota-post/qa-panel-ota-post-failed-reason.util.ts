export type QaPanelOtaPostFailedReasonInput = {
  row_number: number;
  reason: string;
};

export function extractOtaPostFailedReasonsFromProxyResponse(
  proxyResponse: unknown,
): QaPanelOtaPostFailedReasonInput[] {
...
function parseFailedReasons(value: unknown): QaPanelOtaPostFailedReasonInput[] {
  ...
  return value
    ...
    .filter((item): item is QaPanelOtaPostFailedReasonInput => item !== null);
}

function normalizeFailedReason(
  item: unknown,
): QaPanelOtaPostFailedReasonInput | null {
