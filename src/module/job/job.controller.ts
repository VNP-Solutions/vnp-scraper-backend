...
export class JobController {
  ...
  @ApiOperation({
    ...
    description:
      'Accepts an array of one or more job IDs and returns a ZIP file containing one CSV per job. Each CSV is named "{OTA}-{property}-{startDate}-{endDate}.csv" and has one row per job item, with columns populated according to the OTA provider (Expedia / Booking / Agoda). Booking rows always have "N/A" for Check In / Check Out. If the job is Expedia, the Expedia-only columns are appended after the static set: `Card Activity`, `Calculated Amount to Charge`, `Amount Match`, dynamic `Card Activity Approved Amount K` columns, and dynamic `Transaction K` column groups — one 5-cell group per authorization (approved AND declined) followed immediately by one 5-cell group per settlement, all sharing the SAME `K` counter (e.g. 2 authorizations + 1 settlement on a card renders `Transaction 1`, `Transaction 2` for the authorizations, then `Transaction 3` for the settlement). Authorization-sourced groups are `Transaction K Auth Date` / `Posted Date` / `Auth Code` / `Amount` / `Status / Decline Reason` (`Posted Date` is always "N/A" — an authorization never carries its own posted date). Settlement-sourced groups are `Transaction K Transaction Date` / `Post Date` / `Auth Code` / `Reference Number` / `Amount`. The two sources are never cross-matched by `authCode` — only the numbering is shared. Card Number, Expiry Date, CVV, and the `Transaction K` date/Auth Code/Reference Number cells use the Excel `="..."` text-formula trick so Excel preserves them as text. The zip itself is named "job-exports-{D Month YYYY-HH.MM AM/PM}.zip" (e.g. "job-exports-23 April 2026-04.44 PM.zip"). A dot is used instead of ":" in the time so the name is valid on Windows, macOS, and Linux. To export a single job directly as a plain CSV (no zip), use GET /jobs/:id/export-master.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Resolves every non-archived job whose recurring_id and recurring_report_bucket_id match the given query params, then returns a SINGLE combined CSV file containing every job item across all matching jobs (not a ZIP of per-job CSVs). The headers are the union across all OTAs in the bucket: if any Expedia job is included, the Expedia-only columns (Card Activity, Calculated Amount to Charge, Amount Match, dynamic Approved Amount K, and dynamic "Transaction K" column groups — one 5-cell group per authorization (Auth Date / Posted Date / Auth Code / Amount / Status / Decline Reason, Posted Date always "N/A") followed immediately by one 5-cell group per settlement (Transaction Date / Post Date / Auth Code / Reference Number / Amount), all sharing the SAME K counter — appear in the file; non-Expedia rows leave those cells blank. The file is named "job-exports-{D Month YYYY-HH.MM AM/PM}.csv".',
  })
  @ApiQuery({
    name: 'recurring_id',
    required: true,
